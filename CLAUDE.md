...
|--------|---------|---------|
| dev-flow | 6.3.0 | Development workflow: brainstorm → plan → implement (5-gate pipeline + agentic loop + Ralph Loop) → commit → PR → release + automated code review (P0-P3) + agent-team + cross-platform team + Markdown-first knowledge vault + Notion pipeline + product brain + rules distribution |
