...
**70-20-10 原则**:
- ✅ **70% 任务用 sonnet**（默认）- 平衡性能和成本
- ✅ **20% 简单任务用 haiku** - lint、格式化、快速检查
- ✅ **10% 关键任务用 opus** - 安全审计、复杂规划、架构设计

**优化策略**:
1. **默认 sonnet**: 大多数任务无需指定 model 参数
2. **显式 haiku**: 明确简单任务（< 5分钟）才用 haiku
3. **谨慎 opus**: 只在需要最强推理能力时使用
